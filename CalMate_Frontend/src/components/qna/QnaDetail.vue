  ...
  <div class="qna-detail-wrap" v-if="item">
    ...
    <section class="summary">
      ...
      <div class="summary-actions">
        <button v-if="!item.answer" class="btn ghost" type="button" @click="editQuestion">문의 수정</button>
        <button class="btn danger" type="button" @click="deleteQuestion">문의 삭제</button>
      </div>
    </section>
...
function goBack() { router.push({ name: 'dashboard-qna-list' }) }
...
function editQuestion(){ if (!item.value) return; router.push({ name: 'dashboard-qna-new', query: { id: item.value.id } }) }
...
function addComment(){
  ...
  const ok = persistUpdate((updated) => {
    ...
    arr.unshift({ id: `c_${Date.now()}`, content: txt, createdAt: new Date().toISOString(), replies: [] })
...
function addReply(commentId, text){
  ...
  persistUpdate((updated) => {
    ...
    if (target){
      ...
      target.replies.push({ id: `r_${Date.now()}`, content: t, createdAt: new Date().toISOString() })
    }
    updated.comments = arr
  })
}
